...
class Request < Xml
  ...
  "idebit_refund","recur_update","card_verification","reauth"]
...
class Reauth < Xml
  @@keys = ["order_id","cust_id","amount","orig_order_id","txn_number","crypt_type","cust_info","avs_info","cvd_info"]
  make_attrs(Reauth,@@keys)

  def to_xml(indent="")
    to_xml_low("reauth",@@keys,indent)
  end
end

class CardVerification < Xml
  @@keys = ["order_id","cust_id","pan","expdate","crypt_type","avs_info","cvd_info"]
  make_attrs(CardVerification,@@keys)

  def to_xml(indent="")
    to_xml_low("card_verification",@@keys,indent)
  end
end
...
class Completion < Xml
  @@keys = ["order_id","comp_amount","txn_number","crypt_type","ship_indicator"]
...
class Purchasecorrection < Xml
  @@keys = ["order_id","txn_number","crypt_type","ship_indicator"]
...
class IdebitPurchase < Xml
  @@keys = ["order_id", "amount", "idebit_track2","dynamic_descriptor"]
...
class AVSInfo < Xml
  @@keys = ["avs_street_number","avs_street_name","avs_zipcode","avs_email","avs_hostname","avs_browser","avs_shiptocountry","avs_shipmethod","avs_merchprodsku","avs_custip","avs_custphone"]
...
class Receipt < Xml
        ...
        "HostTotals","InitData","DisplayText","ReceiptText",
